...
---
title: "Module 3: Deterministic Blocking"
...
Review the following evaluation metrics and why we use these:
...
Make sure that you're able to write down the confusion matrix as we have previously defined. 
...
```{r, eval=TRUE, message=FALSE, warning=FALSE, fig.width=3.5}
...
recall <- function(block.labels, IDs) {
  # confusion matrix is a sparse table 
  ct = xtabs(~IDs+block.labels)
  
  # Number of true positives
  # choose() function calculates the number of sets with n elements that can be chosen from a set with k elements.
  TP = sum(choose(ct, 2))
...
id08 <- italy08$id
id10 <- italy10$id
id <- c(italy08$id, italy10$id) # combine the unique ids
...
- What are some examples of blocking methods. See if you can think of a few. 
