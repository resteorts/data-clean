...
False Positive (TP): The model incorrectly predicted a positive match (the actual outcome was positive).
...
\frame{
\frametitle{True Negatives}

These record pairs are classified as non-matches, but they are true matches. 
...
These record pairs are classified as non-matches, however they are true matches.
...
\begin{itemize}
\item False positive rate (FPR) = $\dfrac{FP}{FP + FN}$
\begin{itemize}
...
\end{itemize}
\item False negative rate (FNR) = specificity = $\dfrac{TN}{TN + FP}$
\end{itemize}
\vspace*{2em}
