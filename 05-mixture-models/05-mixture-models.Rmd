...
& = \prod_{i=1}^n \sum_{k=1}^K p(x_i, z_i = k \mid \mu_1, \ldots, \mu_K) \; \text{marg. over class}\\
...
\sum_{i=1}^n R_{ik}^{(t-1)} (x_i - \mu_k^{(t)})) = 0 \implies$$

$$\textcolor{blue}{\mu_k^{(t)} = \sum_{i=1}^n w_i x_i}
$$
...
1. I would like to write down the general case if possible. This might be another module as the notation needs to be multivatiate. 
