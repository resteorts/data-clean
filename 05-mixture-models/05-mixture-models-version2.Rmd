...
E-step: Fill in the hidden class labels. M-step: Apply standard MLE (or
other approaches) to complete the data

The algorithm always converges to a local optima of the likelihood.

# Background/Notation review

-   Observations $x_{1:n}$.
-   K total classes
-   $P(Z = i) = \pi_k$ (for $i=1,\ldots, K$)
-   Common variance $\sigma^2.$
-   Want to learn $\mu_1, \ldots, \mu_K$
...
\begin{align}
p(x_1, \ldots, x_n)
& = \prod_{i=1}^n p(x_i \mid \mu_1, \ldots, \mu_K) \\
& = \prod_{i=1}^n \sum_{k=1}^K p(x_i, z_i = k \mid \mu_1, \ldots, \mu_K) \\
& = \prod_{i=1}^n \sum_{k=1}^K p(x_i \mid z_i = k \mid \mu_1, \ldots, \mu_K) p(z_i = k) \\
& \propto \prod_{i=1}^n \sum_{k=1}^K \exp(-\frac{1}{2\sigma^2} (x_i - \mu_k)^2) \pi_k
\end{align}
```
# EM Algorithm
...
& \quad \times
[
\log P(x_i \mid z_i = k, \theta^{t}) + \log P(z_i = k \mid \theta^{t})
] \\
\end{align}
...
\exp(-\frac{1}{2\sigma^2} (x_i - \mu_k^{t-1})^2), \implies$$

$$\frac{\partial Q(\mu_k^t \mid \theta^{t-1})}{\partial \mu_k^{t}} = \frac{-1}{\sigma^2}\sum_{i=1}^n R_{ik}^{t-1} (x_i - \mu_k^{t})^2) = 0 \implies$$

$$\mu_k^t = \sum_{i=1}^n w_i x_i
$$
...
<!-- If you have seen k-means clustering, this is equivalent to assigning clusters to each data point using k-means in a soft way, meaning the clusters can overlap.  -->
...
1.  Simulate data from the mixture model on the previous slide, which
...
2.  Write a function to compute the log-likelihood of incomplete data
    (assuming the parameters are known).
...
Compute the likelihood $P(X_i|Z_i=0)$ and $P(X_i|Z_i=1)$ and store these
in a matrix $L.$

# Exercise

Implement the E and M step in a function called `emIteration`. Then
evaluate the EM and verify that your estimates are 0.29 and 0.71,
respectively.
...
The log-likelihood is strictly increasing, meaning that we have reached
a local maxima.
