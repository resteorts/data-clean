...
We will review k-means clustering in this lecture. 

K-means clustering algorithm
===
\begin{itemize}
\item K-means clustering: simple approach for partitioning a data set into K distinct, non-overlapping clusters. 
\begin{enumerate}
...
We will consider various simulations of data to see where k-means performs well and where it struggles. 

Simulation Study 1
===

Generating 150 data points from three clusters that are well seperated (and have a Gaussian distribution). 
...
library(gridExtra)
n <- 150
cluster1 <- matrix(rnorm(n, mean = 0, sd = 0.5), ncol = 2)
cluster2 <- matrix(rnorm(n, mean = 3, sd = 0.5), ncol = 2)
cluster3 <- matrix(rnorm(n, mean = 6, sd = 0.5), ncol = 2)
...
data$gmm_cluster <- factor(gmm_model$classification)
...
p_gmm <- ggplot(data, aes(x = x, y = y, color = gmm_cluster)) +
  geom_point() +
  labs(title = "GMM Clustering Results") +
  theme_minimal()

# Display plots side by side
library(gridExtra)
grid.arrange(p_original, p_gmm, ncol = 2)
```

Simulation Study 2
===

Generating 150 data points from moon shaped data, i.e. it's not spherical. 

Results: Simulation Study 2
===

```{r, echo = FALSE}
# Generate synthetic data with non-spherical clusters
n <- 150
theta <- seq(0, 2*pi, length.out = n)
cluster1 <- cbind(2 * cos(theta) + rnorm(n, 0, 0.2), 2 * sin(theta) + rnorm(n, 0, 0.2))
cluster2 <- cbind(6 + 0.5 * cos(theta) + rnorm(n, 0, 0.1), 3 + 0.5 * sin(theta) + rnorm(n, 0, 0.1))
cluster3 <- cbind(9 + cos(theta) + rnorm(n, 0, 0.3), 9 + sin(theta) + rnorm(n, 0, 0.3))
data <- rbind(cluster1, cluster2, cluster3)
data <- as.data.frame(data)
colnames(data) <- c("x", "y")
...
theta1 <- runif(n, 0, pi)
theta2 <- runif(n, -pi, 0)
moon1 <- cbind(cos(theta1), sin(theta1))
moon2 <- cbind(1 - cos(theta2), sin(theta2) - 0.5)
moon_data <- rbind(moon1, moon2)
moon_data <- as.data.frame(moon_data)
colnames(moon_data) <- c("x", "y")

# Apply K-means Clustering
K <- 2
kmeans_moon <- kmeans(moon_data, centers = K, nstart = 25)
moon_data$cluster <- factor(kmeans_moon$cluster)

# Plot Moon-Shaped Data
p_moon_orig <- ggplot(moon_data, aes(x = x, y = y)) +
  geom_point() +
  labs(title = "Original Moon-Shaped Data") +
  theme_minimal()

p_moon_cluster <- ggplot(moon_data, aes(x = x, y = y, color = cluster)) +
  geom_point() +
  labs(title = "K-means Clustering on Moon-Shaped Data") +
  theme_minimal()

# Display Moon-Shaped Data Plots
grid.arrange(p_moon_orig, p_moon_cluster, ncol = 2)
```

Simulation Study 3
===

Generate more half moons. 
...
# Generate Four Half-Moon Shaped Clusters
n_moon <- 150
theta1 <- runif(n_moon, 0, pi)
theta2 <- runif(n_moon, -pi, 0)
theta3 <- runif(n_moon, 0, pi)
theta4 <- runif(n_moon, -pi, 0)

moon1 <- cbind(5 + cos(theta1), 5 + sin(theta1))
moon2 <- cbind(5 + 1 - cos(theta2), 5 + sin(theta2) - 1)
moon3 <- cbind(5 + 2 + cos(theta3), 5 + sin(theta3) - 2)
moon4 <- cbind(5 + 3 - cos(theta4), 5 + sin(theta4) - 3)

moon_data <- rbind(moon1, moon2, moon3, moon4)
moon_data <- as.data.frame(moon_data)
colnames(moon_data) <- c("x", "y")

# Apply K-means Clustering
K <- 4
kmeans_moons <- kmeans(moon_data, centers = K, nstart = 25)
moon_data$cluster <- factor(kmeans_moons$cluster)

# Plot Original Moon-Shaped Data
p_moon_orig <- ggplot(moon_data, aes(x = x, y = y)) +
  geom_point() +
  labs(title = "Original Moon-Shaped Data (Four Half Moons)") +
  theme_minimal()
...
# Apply Gaussian Mixture Model (GMM)
gmm_model <- Mclust(moon_data, G = 4)
moon_data$cluster <- factor(gmm_model$classification)
...
# Plot GMM Clustering Results
p_moon_gmm <- ggplot(moon_data, aes(x = x, y = y, color = cluster)) +
  geom_point() +
  labs(title = "GMM Clustering on Moon-Shaped Data") +
  theme_minimal()

# Display Plots Side by Side
grid.arrange(p_moon_orig, p_moon_gmm, ncol = 2)
```

Moon Shaped Applications
===

1. In some astronomical applications, moon-shaped distributions can occur due to the periodic and elliptical nature of orbits. Feigelson, E. D., & Babu, G. J. (2010), Pössel, M. (2020).

2. Biological. Examples might be distributions of species in 
different habitats or the clustering of cells in tissue samples. 
Antão, L. H., Magurran, A. E., & Dornelas, M. (2021), 

3. Economics: moon-shaped distributions can sometimes be seen in datasets related to market trends or consumer behavior, where two distinct groups of data points such as (high and low spenders) form crescent shapes. Jacobi, A., & Tzur, J. (2024).

4. Social networks analysis can have these distributions as well. Wasserman, S., & Faust, K. (1994), Newman, M. E. J. (2010).

Simulation Study 4
===

Generated a double ring shaped distribution (small ring inside a larger ring) and apply k-means clustering. 

Results: Simulation Study 4
===

```{r, echo = FALSE}
n <- 150
theta <- seq(0, 2*pi, length.out = n)
ring1 <- cbind(cos(theta) + rnorm(n, 0, 0.1), sin(theta) + rnorm(n, 0, 0.1))
ring2 <- cbind(3*cos(theta) + rnorm(n, 0, 0.1), 3*sin(theta) + rnorm(n, 0, 0.1))
double_ring_data <- rbind(ring1, ring2)
double_ring_data <- as.data.frame(double_ring_data)
colnames(double_ring_data) <- c("x", "y")

# Apply K-means Clustering
K <- 2
kmeans_double_ring <- kmeans(double_ring_data, centers = K, nstart = 25)
double_ring_data$cluster <- factor(kmeans_double_ring$cluster)

# Plot Double Ring-Shaped Data
p_double_ring_orig <- ggplot(double_ring_data, aes(x = x, y = y)) +
  geom_point() +
  labs(title = "Original Double Ring-Shaped Data") +
  theme_minimal()

p_double_ring_cluster <- ggplot(double_ring_data, aes(x = x, y = y, color = cluster)) +
  geom_point() +
  labs(title = "K-means Clustering on Double Ring-Shaped Data") +
  theme_minimal()

# Display Double Ring-Shaped Data Plots
grid.arrange(p_double_ring_orig, p_double_ring_cluster, ncol = 2)
```
...
```{r, echo = FALSE, warning = FALSE, message = FALSE}
# Apply Gaussian Mixture Model (GMM)
gmm_model <- Mclust(double_ring_data, G = 2)
double_ring_data$gmm_cluster <- factor(gmm_model$classification)

# Plot Original Double Ring-Shaped Data
p_double_ring_orig <- ggplot(double_ring_data, aes(x = x, y = y)) +
  geom_point() +
  labs(title = "Original Double Ring-Shaped Data") +
  theme_minimal()
...
# Plot GMM Clustering Results
p_double_ring_gmm <- ggplot(double_ring_data, aes(x = x, y = y, color = gmm_cluster)) +
  geom_point() +
  labs(title = "GMM Clustering on Double Ring-Shaped Data") +
  theme_minimal()

# Display Double Ring-Shaped Data Plots
grid.arrange(p_double_ring_orig, p_double_ring_gmm, ncol = 2)
```

Real World Double Ring Shaped Distributions
===

Double ring-shaped distributions are often found in fields with spatial patterns, such as laser technology, urban networks, material processing, biophysics and nanotechnology. 

One distribution that can handle circular data is the von Mises distribution (or the circular normal distribution). 
